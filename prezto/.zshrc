...
if [[ -a '/usr/local/share/zsh-completions' ]]; then
  echo HELLO
...
# Compile Bash autocomplete
if [[ -a '/usr/local/etc/bash_completion.d' ]]; then
  autoload -Uz bashcompinit && bashcompinit
  for file in `find /usr/local/etc/bash_completion.d -type f`; do source $file; done
fi
